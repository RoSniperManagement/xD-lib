...
-- Each theme carries two accents so gradients, ambient light blobs, and
-- active states can read as a duotone rather than one flat hue.

Themes.MidnightGlass = {
	Name = "Midnight Glass",
	Accent = Color3.fromRGB(122, 148, 255),
	Accent2 = Color3.fromRGB(196, 122, 255),
	AccentDim = Color3.fromRGB(84, 100, 200),
	Background = Color3.fromRGB(9, 10, 15),
	Elevated = Color3.fromRGB(17, 18, 26),
	Panel = Color3.fromRGB(28, 30, 42),
	PanelLight = Color3.fromRGB(44, 47, 64),
	Border = Color3.fromRGB(78, 83, 110),
	Text = Color3.fromRGB(240, 241, 248),
	SubText = Color3.fromRGB(160, 164, 184),
	MutedText = Color3.fromRGB(112, 116, 136),
	Success = Color3.fromRGB(94, 224, 156),
	Warning = Color3.fromRGB(245, 190, 92),
	Danger = Color3.fromRGB(240, 98, 116),
}

Themes.NebulaPurple = {
	Name = "Nebula Purple",
	Accent = Color3.fromRGB(178, 118, 255),
	Accent2 = Color3.fromRGB(255, 120, 196),
	AccentDim = Color3.fromRGB(128, 90, 200),
	Background = Color3.fromRGB(12, 8, 18),
	Elevated = Color3.fromRGB(20, 14, 28),
	Panel = Color3.fromRGB(33, 24, 46),
	PanelLight = Color3.fromRGB(50, 37, 68),
	Border = Color3.fromRGB(88, 66, 116),
	Text = Color3.fromRGB(242, 236, 250),
	SubText = Color3.fromRGB(170, 158, 188),
	MutedText = Color3.fromRGB(120, 110, 138),
	Success = Color3.fromRGB(102, 232, 168),
	Warning = Color3.fromRGB(248, 194, 104),
	Danger = Color3.fromRGB(245, 102, 136),
}

Themes.Solar = {
	Name = "Solar",
	Accent = Color3.fromRGB(255, 166, 72),
	Accent2 = Color3.fromRGB(255, 106, 96),
	AccentDim = Color3.fromRGB(210, 128, 55),
	Background = Color3.fromRGB(14, 12, 10),
	Elevated = Color3.fromRGB(22, 19, 16),
	Panel = Color3.fromRGB(36, 31, 26),
	PanelLight = Color3.fromRGB(52, 45, 37),
	Border = Color3.fromRGB(92, 79, 64),
	Text = Color3.fromRGB(248, 243, 235),
	SubText = Color3.fromRGB(182, 171, 156),
	MutedText = Color3.fromRGB(128, 118, 104),
	Success = Color3.fromRGB(126, 216, 126),
	Warning = Color3.fromRGB(252, 204, 96),
	Danger = Color3.fromRGB(240, 104, 92),
}
...
function xD:SaveTheme(name)
	local ok = pcall(function()
		...
		local data = {}
		for key, value in self.Theme do
...
-- The GUI root uses ZIndexBehavior.Sibling, where a descendant always draws
-- above its ancestor. Decoration that must sit *behind* an element therefore
-- has to be a lower-ZIndex sibling, not a child. These helpers mirror a
-- target's geometry onto such a sibling.

-- Center of `target` expressed as a position for an AnchorPoint(0.5, 0.5) node.
local function CenterPositionOf(target)
...
local function ExpandedSizeOf(target, pad)
	local size = target.Size
	return UDim2.new(size.X.Scale, size.X.Offset + pad, size.Y.Scale, size.Y.Offset + pad)
end

-- Drop shadow rendered as a sibling behind `target`, so it survives the
-- target clipping its own descendants. Tracks moves/resizes.
function Glass.Shadow(target, elevation)
	elevation = elevation or 1
	local pad = (14 + elevation * 6) * 2
	...
	local shadow = New("ImageLabel", {
		...
		ImageTransparency = 0.5 - math.min(elevation * 0.03, 0.15),
		ScaleType = Enum.ScaleType.Slice,
		SliceCenter = Rect.new(23, 23, 277, 277),
		AnchorPoint = Vector2.new(0.5, 0.5),
		Position = CenterPositionOf(target),
		Size = ExpandedSizeOf(target, pad),
		ZIndex = target.ZIndex - 1,
		Parent = target.Parent,
	})

	local function sync()
		shadow.Position = CenterPositionOf(target)
		shadow.Size = ExpandedSizeOf(target, pad)
	end
...
function Glass.Panel(props)
	...
	-- Border: bright at the top edge (light source above), fading to nearly
	-- invisible at the bottom — this single detail is what sells "pane of
	-- glass" over "grey rectangle".
...
-- Soft accent glow rendered as a sibling *behind* `target` (checked toggle,
-- active tab indicator, slider knob). It tracks the target automatically, so
-- glows follow animated controls without extra bookkeeping.
function Glass.Glow(target, color, pad, transparency)
	pad = pad or 24
	...
	local function sync()
		glow.Position = CenterPositionOf(target)
		glow.Size = ExpandedSizeOf(target, pad)
	end
	target:GetPropertyChangedSignal("Position"):Connect(sync)
	target:GetPropertyChangedSignal("Size"):Connect(sync)
	target.Destroying:Connect(function()
...
-- Ambient light behind the glass. Frosted panels only read as glass when
-- there is something colorful underneath to diffuse, so each window gets
-- large duotone light blobs that drift slowly behind the content layer.
function Glass.Backdrop(parent, corner)
	local theme = xD.Theme
	...
	local backdrop = New("Frame", {
		...
		New("UICorner", { CornerRadius = corner or UDim.new(0, 18) }),
	})
	xD:RegisterThemable(backdrop, "BackgroundColor3", "Background")

	local blobs = {
		{ Key = "Accent", Pos = UDim2.fromScale(0.12, 0.05), Size = UDim2.fromScale(0.85, 0.95), Drift = 0.05 },
		{ Key = "Accent2", Pos = UDim2.fromScale(0.78, 0.62), Size = UDim2.fromScale(0.8, 0.9), Drift = -0.04 },
		{ Key = "Accent", Pos = UDim2.fromScale(0.55, 1.05), Size = UDim2.fromScale(0.6, 0.7), Drift = 0.03 },
	}

	for index, spec in blobs do
		local blob = New("ImageLabel", {
			Name = "Blob" .. index,
			BackgroundTransparency = 1,
			Image = "rbxassetid://5028857084",
			ImageColor3 = theme[spec.Key] or theme.Accent,
		...
		xD:RegisterThemable(blob, "ImageColor3", spec.Key)

		-- slow, non-looping-feeling drift so the surface is never fully static
		local basePos = spec.Pos
		Utils.Tween(
			blob,
			TweenInfo.new(9 + index * 2.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true),
			{ Position = UDim2.fromScale(basePos.X.Scale + spec.Drift, basePos.Y.Scale - spec.Drift) }
		)
	end
...
-- Slowly rotating duotone rim light around a panel. Reads as a premium
-- "living" border rather than a static 1px outline.
function Glass.AuroraBorder(panel, thickness)
	local theme = xD.Theme
	local stroke = New("UIStroke", {
		Name = "Aurora",
	...
	local gradient = New("UIGradient", {
		Color = ColorSequence.new({
			...
			ColorSequenceKeypoint.new(0.5, theme.Accent2 or theme.Accent),
...
function xD:CreateWindow(config)
	...
	local windowCorner = UDim.new(0, 18)
	...
	-- colorful ambient light behind every translucent surface in the window
	Glass.Backdrop(main, windowCorner)
	Glass.AuroraBorder(main, 1.5)
	...
	local titleBar = New("Frame", {
		...
		New("UICorner", { CornerRadius = windowCorner }),
	})
	...
	-- duotone hairline under the title bar, brightest under the title itself
	local titleRule = New("Frame", {
		BackgroundColor3 = Color3.new(1, 1, 1),
		BorderSizePixel = 0,
	...
	New("UIGradient", {
		Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0, theme.Accent),
			ColorSequenceKeypoint.new(0.45, theme.Accent2 or theme.Accent),
			ColorSequenceKeypoint.new(1, theme.Border),
		}),
		Transparency = NumberSequence.new({
	...
	if config.IconId then
		IconKit.Resolve(config.IconId, UDim2.fromOffset(20, 20), theme.Accent, titleBar).Position = UDim2.new(0, 18, 0.5, -10)
	else
		-- brand mark: a small duotone gradient chip so the corner isn't empty
		...
		New("UIGradient", {
			Color = ColorSequence.new(theme.Accent, theme.Accent2 or theme.Accent),
			Rotation = 55,
			Parent = mark,
		})
		Glass.Glow(mark, theme.Accent, 22, 0.5)
		New("TextLabel", {
			BackgroundTransparency = 1,
			Size = UDim2.fromScale(1, 1),
			Font = Enum.Font.GothamBold,
			Text = string.sub(self_.Title, 1, 1):upper(),
			TextSize = 14,
	...
	local textOffsetX = 52
	local titleLabel = New("TextLabel", {
		BackgroundTransparency = 1,
		Position = UDim2.new(0, textOffsetX, 0, config.SubTitle and 9 or 0),
		Size = UDim2.new(1, -150, 0, config.SubTitle and 18 or 52),
		Font = Enum.Font.GothamBold,
		Text = self_.Title,
		TextSize = 16,
	...
	if config.SubTitle then
		local subLabel = New("TextLabel", {
			BackgroundTransparency = 1,
			Position = UDim2.new(0, textOffsetX, 0, 27),
		...
		xD:RegisterThemable(subLabel, "TextColor3", "MutedText")
	end
	...
	local function makeWindowButton(offsetFromRight, iconName, hoverColor, onClick)
		local btn = New("TextButton", {
			...
			Position = UDim2.new(1, -offsetFromRight, 0.5, -14),
			Text = "",
			...
			New("UICorner", { CornerRadius = UDim.new(0, 9) }),
			New("UIStroke", { Color = theme.Border, Transparency = 0.6 }),
		})
		...
		btn.MouseLeave:Connect(function()
			Utils.FastTween(btn, { BackgroundColor3 = theme.PanelLight, BackgroundTransparency = 0.45 }, 0.15)
		end)
		btn.MouseButton1Click:Connect(onClick)
		return btn
	end

	makeWindowButton(40, "close", theme.Danger, function()
		self_:Destroy()
	end)
	makeWindowButton(74, "minimize", theme.Accent, function()
		self_:Toggle()
	end)
	...
	-- keep the bloom under the tab list (which sits at ZIndex 11) so it reads
	-- as light spilling from behind the rail, not a wash over the buttons
	Glass.Glow(indicator, theme.Accent, 26, 0.35).ZIndex = 10
	...
	Glass.Shadow(dock, 4)
	Glass.Glow(dock, theme.Accent, 30, 0.45)
	local dockIcon = IconKit.Resolve("grip", UDim2.fromOffset(16, 16), Color3.new(1, 1, 1), dock)
...
function Window:Destroy()
	...
	self.Main:Destroy()
	self.Dock:Destroy()
	-- indexed loop rather than generalized iteration: Luau forbids mutating
	-- a table while iterating it
	for i = #xD.Windows, 1, -1 do
		if xD.Windows[i] == self then
			table.remove(xD.Windows, i)
			break
		end
	end
end
...
function TabSection:CreateTab(config, flag)
	...
	-- icon sits in its own tinted chip so the rail has rhythm even when idle
	...
	local iconColor = theme.SubText
	local icon = IconKit.Resolve(config.Icon or "dot", UDim2.fromOffset(13, 13), iconColor, iconChip)
	icon.AnchorPoint = Vector2.new(0.5, 0.5)
	icon.Position = UDim2.fromScale(0.5, 0.5)
	icon.ZIndex = 13
	...
	tab.Button = btn
	tab.Label = label
	tab.IconChip = iconChip
	tab.IconHolder = icon

	local function paintIcon(target, color)
	...
	local function selectTab()
		for _, otherTab in window.Tabs do
			...
			Utils.FastTween(otherTab.IconChip, { BackgroundColor3 = theme.PanelLight, BackgroundTransparency = 0.45 }, 0.15)
			paintIcon(otherTab.IconHolder, theme.SubText)
		end
		page.Visible = true
		Utils.FastTween(label, { TextColor3 = theme.Text }, 0.15)
		Utils.FastTween(btn, { BackgroundTransparency = 0.88 }, 0.15)
		Utils.FastTween(iconChip, { BackgroundColor3 = theme.Accent, BackgroundTransparency = 0.15 }, 0.15)
		paintIcon(icon, Color3.new(1, 1, 1))
		local offsetY = btn.AbsolutePosition.Y - window.Sidebar.AbsolutePosition.Y
			+ (btn.AbsoluteSize.Y - window.Indicator.AbsoluteSize.Y) / 2
	...
	btn.MouseEnter:Connect(function()
		if window.ActiveTab ~= tab then
			Utils.FastTween(btn, { BackgroundTransparency = 0.94 }, 0.12)
			Utils.FastTween(iconChip, { BackgroundTransparency = 0.25 }, 0.12)
		end
	end)
	btn.MouseLeave:Connect(function()
		if window.ActiveTab ~= tab then
			Utils.FastTween(btn, { BackgroundTransparency = 1 }, 0.12)
			Utils.FastTween(iconChip, { BackgroundTransparency = 0.45 }, 0.12)
		end
	end)
...
-- `skipHover` is for components that paint their own background (filled
-- buttons), which would otherwise fight the shared hover wash.
function ComponentHost:_row(config, rightWidth, skipHover)
	config = config or {}
	local theme = xD.Theme
	...
	-- hover lifts the row with a faint accent wash rather than flat grey
	if not skipHover then
		row.MouseEnter:Connect(function()
			Utils.FastTween(row, { BackgroundColor3 = theme.Accent, BackgroundTransparency = 0.9 }, 0.12)
		end)
		row.MouseLeave:Connect(function()
			Utils.FastTween(row, { BackgroundColor3 = theme.PanelLight, BackgroundTransparency = 1 }, 0.12)
		end)
	end
...
function Tab:CreateGroupbox(config)
	...
	if config.Name then
		-- accent bar + title, so each card announces itself instead of
		-- blending into a wall of identical rectangles
		local bar = New("Frame", {
			Name = "TitleBar",
			BackgroundColor3 = theme.Accent,
			BorderSizePixel = 0,
		...
		xD:RegisterThemable(bar, "BackgroundColor3", "Accent")
		...
		xD:RegisterThemable(heading, "TextColor3", "Text")
	end
	...
	local container = New("Frame", {
		...
		Position = UDim2.new(0, 8, 0, config.Name and 32 or 8),
...
function ComponentHost:CreateButton(config, flag)
	...
	local row, title = self:_row(config, 0, true)
	...
	hitbox.MouseEnter:Connect(function()
		Utils.FastTween(row, { BackgroundTransparency = style == 1 and 0.15 or 0.88 }, 0.12)
	end)
	hitbox.MouseLeave:Connect(function()
		Utils.FastTween(row, { BackgroundTransparency = style == 1 and 0 or 1 }, 0.12)
	end)
	hitbox.MouseButton1Down:Connect(function()
		Utils.FastTween(row, { Size = UDim2.new(1, 0, 0, 33) }, 0.08)
	end)
	hitbox.MouseButton1Up:Connect(function()
		Utils.FastTween(row, { Size = UDim2.new(1, 0, 0, 36) }, 0.08)
	end)
...
function ComponentHost:CreateToggle(config, flag)
	...
	-- the duotone gradient only applies in the "on" state, so "off" stays
	-- unmistakably neutral instead of half-tinted
	...
	local switchGlow = Glass.Glow(switch, theme.Accent, 22, value and 0.4 or 1)
	...
	local function apply(animated)
		...
		local targetGlow = element.Value and 0.35 or 1
		if animated then
			...
			Utils.FastTween(switchGlow, { ImageTransparency = targetGlow }, 0.2)
		else
			knob.Position = targetPos
			switch.BackgroundColor3 = targetColor
			switchGlow.ImageTransparency = targetGlow
		end
	end
...
function ComponentHost:CreateSlider(config, flag)
	...
	Glass.Glow(knob, theme.Accent, 18, 0.4)
...
local function BuildDropdown(host, config, flag, searchable)
	...
	local selected = {}
	if multi then
		for _, v in config.CurrentOption or {} do selected[v] = true end
	else
	...
	local function refreshDisplay()
		if multi then
			...
			local chosen
			for item in selected do chosen = item end
	...
	local function buildOptionButton(item, container)
		local optBtn = New("TextButton", {
			BackgroundColor3 = theme.Accent,
			BackgroundTransparency = selected[item] and 0.78 or 1,
		...
		-- selected rows carry an accent tick bar so state survives at a glance
		local tick = New("Frame", {
			BackgroundColor3 = theme.Accent,
			BorderSizePixel = 0,
			Size = UDim2.fromOffset(3, 14),
			Position = UDim2.new(0, 6, 0.5, -7),
			Visible = selected[item] == true,
			ZIndex = 103,
			Parent = optBtn,
		}, { New("UICorner", { CornerRadius = UDim.new(1, 0) }) })
		...
		local optLabel = New("TextLabel", {
			...
			Font = selected[item] and Enum.Font.GothamBold or Enum.Font.Gotham,
			Text = tostring(item),
			TextSize = 12,
			TextColor3 = selected[item] and theme.Text or theme.SubText,
			TextXAlignment = Enum.TextXAlignment.Left,
			ZIndex = 103,
			Parent = optBtn,
		})
		...
		local function paintState()
			...
			optLabel.Font = isSelected and Enum.Font.GothamBold or Enum.Font.Gotham
			optLabel.TextColor3 = isSelected and theme.Text or theme.SubText
			optBtn.BackgroundTransparency = isSelected and 0.78 or 1
		end

		optBtn.MouseEnter:Connect(function()
			Utils.FastTween(optBtn, { BackgroundTransparency = 0.85 }, 0.1)
		end)
		optBtn.MouseLeave:Connect(function()
			Utils.FastTween(optBtn, { BackgroundTransparency = selected[item] and 0.78 or 1 }, 0.1)
		end)
	...
	local function openPopup()
		...
		local popupWidth = math.max(absSize.X, 170)
		local listHeight = math.min(#items * 30 + (searchable and 36 or 8) + 10, 230)

		popup = Glass.Panel({
			Name = "DropdownPopup",
			Size = UDim2.fromOffset(popupWidth, listHeight),
			Position = UDim2.fromOffset(absPos.X + absSize.X - popupWidth, absPos.Y + absSize.Y + 6),
		...
		Glass.AuroraBorder(popup, 1.2)

		-- grow-in so the popup feels attached to the control it came from
		popup.Size = UDim2.fromOffset(popupWidth, 0)
		Utils.Tween(popup, TweenInfo.new(0.18, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
			Size = UDim2.fromOffset(popupWidth, listHeight),
		})
		...
		if searchable then
			...
			local searchIcon = IconKit.Resolve("search", UDim2.fromOffset(12, 12), theme.MutedText, searchHolder)
		...
		popupList = New("ScrollingFrame", {
			BackgroundTransparency = 1,
			Position = UDim2.new(0, 6, 0, searchable and 40 or 6),
			Size = UDim2.new(1, -12, 1, -(searchable and 46 or 12)),
			BorderSizePixel = 0,
			ScrollBarThickness = 3,
			ScrollBarImageColor3 = theme.Border,
			CanvasSize = UDim2.new(0, 0, 0, 0),
		...
		if searchable and searchBox then
			searchBox:GetPropertyChangedSignal("Text"):Connect(function()
				...
				for item, btn in optionButtons do
	...
	function element:Set(newValue)
		if multi then
			selected = {}
			for _, v in newValue do selected[v] = true end
		else
...
function ComponentHost:CreateColorPicker(config, flag)
	...
	local function openPopup()
		...
		popup = Glass.Panel({
			...
			Position = UDim2.fromOffset(absPos.X + absSize.X - 200, absPos.Y + absSize.Y + 6),
			Corner = UDim.new(0, 12),
			Elevation = 4,
			ZIndex = 100,
			Parent = xD.RootGui,
...
function xD:Notify(config)
	...
	-- The stack is driven by a UIListLayout, which overrides Position on its
	-- children — so each toast slides inside its own wrapper slot instead.
	...
	Glass.Glow(accentBar, accent, 16, 0.55)
...
function xD:ListConfigs()
	...
	pcall(function()
		if fs.isfolder(self.ConfigFolder) then
			for _, file in fs.listfiles(self.ConfigFolder) do
...
-- `game:BindToClose` is a server-only API and throws when called from a
-- client context (LocalScript / executor), which is where this library
-- always runs. The client-side equivalent teardown signal is the local
-- player being removed (leaving/teleporting away from the game).
local function RunOnDestroyHooks()
	if xD._onDestroy then
		for _, cb in xD._onDestroy do
			pcall(cb)
		end
	end
end
